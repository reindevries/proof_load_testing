...
// load application. For each step a Markov-chain Monte Carlo (MCMC) simulation is performed by 
// which the reliability during and after a successful load step is calculated. The output of this 
// code is provided in Table 5.1 of the dissertation.
//
...
int main(int argc, char* argv[]) {
	...
	// Eurocode LM1 load effect: shear force at 1 m from support
	double L = 9.0;
	double dummy, Q_k_LM1;
	load_effect_LM1(L, dummy, Q_k_LM1);
	Q_k_LM1 /= 1e3;
	...
	auto rv_G_DL = rdv::rv_normal::mv(356.0, 0.05);
	auto rv_G_SDL = rdv::rv_normal::mv(59.3, 0.10);
	auto rv_C_0Q = rdv::rv_lognormal::mv(1.1, 0.10);
	auto rv_Q = rdv::rv_gumbel_max::mv(390.0, 0.035);
	...
	// Obtain characteristic value from WIM statistical description
	double Q_k_WIM = rv_Q.quantile(0.999);
	std::cout << "Q_k_WIM = " << Q_k_WIM << " kN\n";
	
	// Define target loads and indicator readings
	std::vector<double> m_Q_PL = {Q_k_WIM, 
			1.2 * Q_k_WIM, 1.4 * Q_k_WIM, 1.6 * Q_k_WIM, 1.8 * Q_k_WIM};
	std::vector<double> w_max_w = {0.034, 0.038, 0.048, 0.096, 0.160};
	
	// Obtain sample mean and standard deviation
	std::cout << "Statistical properties of strength ratio X\n";
	...
	size_t n_X = 5;  // based on five tests
	std::vector<double> m_X, s_X;
	
	for (size_t i = 0; i < size(m_Q_PL); i++) {
		double w = w_max_w[i];
		m_X.push_back(w < 0.85 ? 1.5  * exp(-3.0 * w) + 0.88 : 1.0);
		s_X.push_back(w < 1.1  ? 0.53 * exp(-2.1 * w) - 0.05 : 1e-8);
		
		std::cout << std::setw(15) << m_Q_PL[i] / Q_k_WIM <<
		             std::setw(15) << m_Q_PL[i] / Q_k_LM1 <<
		             std::setw(15) << w_max_w[i] <<
		             std::setw(15) << m_X[i] <<
		             std::setw(15) << s_X[i] <<
		             std::setw(15) << s_X[i] / m_X[i] << "\n";
	}
	...
	for (size_t i = 0; i < size(m_Q_PL); i++) {
		...
		auto rv_X = rdv::rv_students_t(n_X - 1, m_X[i], s_X[i] * sqrt(1.0 + 1.0/n_X));
