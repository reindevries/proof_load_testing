...
//
// This code calculates transfer factors for case study 1, considering shear. The flag 
...
// in each configuration and the n = N case results in the transfer factors. The results for shear 
// are provided in Table 6.4 of the dissertation.
//
...
inline void calculate_reliability(size_t N, size_t n, double m_Q_PL, double& β) {
	...
	// Random variables
	auto rv_θ_R = rdv::rv_lognormal::mv(1.0, 0.1);     // value for shear
	auto rv_m_R = rdv::rv_lognormal::mv(2.7, 0.5);     // 1.5 times the mean load effect
	auto rv_V_R = rdv::rv_lognormal::mv(0.1, 0.1);     // value for shear
	auto rv_θ_E = rdv::rv_lognormal::mv(1.0, 0.1);     // model uncertainty load effect
	auto rv_G = rdv::rv_normal::mv(0.8, 0.07);         // value for shear
	auto rv_C_0Q = rdv::rv_lognormal::mv(1.0, 0.1);    // stat uncertainty traffic load, no DAF
	auto rv_Q = rdv::rv_gumbel_max::mv(1.0, 0.035);    // value for shear, small spans
	...
	#pragma omp parallel reduction(+:post_samples, failures)
	{
		...
		for (size_t i_sample = 0; i_sample < samples; i_sample++) {
			...
			// Subject all N components to one year of traffic load (in-service proven strength)
			bool failed = false;
			double Q = rv_Q(gen);
			for (size_t i = 0; i < N; i++) {
			...
			// Subject all N components to a future year of traffic load
			failed = false;
			post_samples++;
			Q = rv_Q(gen);
			for (size_t i = 0; i < N; i++) {
...
int main(int argc, char *argv[]) {
	...
	for (size_t n : ns) {
		for (size_t N : Ns) {
			...
			double start = (N == n ? 1.4 : 1.6);
			double step  = (N == n ? 0.1 : 0.4);
